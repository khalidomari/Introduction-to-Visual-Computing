...
public class QuadGraph {
	...
	List<int[]> findCycles() {
		...
		// filter cycles list
		int width = ImageProcessing.img.width;
		int height = ImageProcessing.img.height;
		float maxArea = width * height;
		float minArea = 0;
		for (int i = 0; i < cycles.size(); i++) {
			PVector[] v = new PVector[4];
			v=getPVector(i);
			
			// delete if not Convex or ValidArea or nonFlatQuad
			if (!isConvex(v[0], v[1], v[2], v[3])
					|| !validArea(v[0], v[1], v[2], v[3], maxArea, minArea)
		...
		int maxAreaIdx=0;
		PVector[] vMax = new PVector[4];
		vMax=getPVector(0);
		float maxA= calculateArea(vMax[0], vMax[1],vMax[2], vMax[3]);
		for (int i = 0; i < cycles.size(); i++) {
			PVector[] v = new PVector[4];
			v=getPVector(i);
			float currentArea= calculateArea(v[0], v[1],v[2], v[3]);
		...
		List<int[]> cycles2 = new ArrayList<int[]>();
		cycles2.add(cycles.get(maxAreaIdx));
	...
	public PVector[] getPVector(int i){
		PVector l1 = ImageProcessing.linesList.get(cycles.get(i)[0]);
		PVector l2 = ImageProcessing.linesList.get(cycles.get(i)[1]);
		PVector l3 = ImageProcessing.linesList.get(cycles.get(i)[2]);
		PVector l4 = ImageProcessing.linesList.get(cycles.get(i)[3]);
		
		
		PVector c12 = ImageProcessing.intersection(l1, l2);
		PVector c23 = ImageProcessing.intersection(l2, l3);
		PVector c34 = ImageProcessing.intersection(l3, l4);
		PVector c41 = ImageProcessing.intersection(l4, l1);
	...
	public float calculateArea(PVector c1, PVector c2, PVector c3, PVector c4) {
		PVector v21 = PVector.sub(c1, c2);
		PVector v32 = PVector.sub(c2, c3);
		PVector v43 = PVector.sub(c3, c4);
		PVector v14 = PVector.sub(c4, c1);
